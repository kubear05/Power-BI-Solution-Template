...
function AES-Encrypt
{
    param([Parameter(Mandatory=$true, Position=0)][ValidateNotNullOrEmpty()] $message,
          ...
          [Parameter(Mandatory=$true, Position=2)][ValidateNotNullOrEmpty()] $salt
         )
    ...
    $rfc_derivative = New-Object -TypeName System.Security.Cryptography.Rfc2898DeriveBytes -ArgumentList $key, $salt_bytes
    $aes.Key = $rfc_derivative.GetBytes($aes.KeySize / 8)
    $rfc_derivative.Dispose()

    $encryptor = $aes.CreateEncryptor()
    
    $memory_stream = New-Object -TypeName System.IO.MemoryStream
    $crypto_stream = New-Object System.Security.Cryptography.CryptoStream -ArgumentList $memory_stream,  $encryptor, ([System.Security.Cryptography.CryptoStreamMode]::Write)
...
function AES-Decrypt
{
    ...
    #  PBKDF2 standard with HMACSHA1 for password-based key generation
    $rfc_derivative = New-Object System.Security.Cryptography.Rfc2898DeriveBytes -ArgumentList $key, $salt_bytes
    $aes.Key = $rfc_derivative.GetBytes($aes.KeySize / 8)
    $rfc_derivative.Dispose()

    $decryptor = $aes.CreateDecryptor()
    $memory_stream = New-Object -TypeName System.IO.MemoryStream -ArgumentList @(,$encrypted_bytes)
    $crypto_stream = New-Object -TypeName System.Security.Cryptography.CryptoStream -ArgumentList $memory_stream, $decryptor, ([System.Security.Cryptography.CryptoStreamMode]::Read)

    $decrypted_bytes = New-Object Byte[] $memory_stream.Length
    #  Write the iv + cipherText array to the crypto stream and flush it
    $crypto_stream.Read($decrypted_bytes, 0, $decrypted_bytes.Length) | Out-Null
    $decryptor.Dispose()
