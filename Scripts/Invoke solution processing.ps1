<# Description : invokes processing of a solution
...
if ($etl -eq "Informatica")
{
    ...
    try
    {
        ...
        $runID = INFA-Start-Job -TaskName $solution_name
        INFA-Logout

        $q = "DELETE FROM smgt.configuration WHERE configuration_group='informatica' AND configuration_subgroup='runtime' AND [name]='last_run_id'"
        Invoke-Sqlcmd -ServerInstance $sqlServer -Database $sqlDatabase -Query $q
        $q = "INSERT smgt.configuration(configuration_group, configuration_subgroup, [name], [value]) VALUES('informatica', 'runtime', 'last_run_id', '" + $runID + "')"
        Invoke-Sqlcmd -ServerInstance $sqlServer -Database $sqlDatabase -Query $q
    }
    catch
    {
    ...
    Write-Host "Requested solution processing."
}
elseif ($etl -eq "Scribe")
{
    ...
    try
    {
        ...
        # Invoke processing
        $response = SU-ProcessSolution -SolutionID $solution_id -OrganizationID $org_id -authenticationHeader $headers
        }
        catch
        {
