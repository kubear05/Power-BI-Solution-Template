...
/****** Object:  Job [Data load and processing]    Script Date: 1/21/2016 5:05:20 PM ******/
BEGIN TRANSACTION;
DECLARE @ReturnCode INT;

IF EXISTS (SELECT job_id FROM msdb.dbo.sysjobs WHERE [name] = N'Data load and processing')
BEGIN
    EXEC @ReturnCode = sp_delete_job @job_name= 'Data load and processing';
...
BEGIN
    EXEC @ReturnCode = sp_delete_job @job_name= 'Save credential';
    IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback;
END;
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Encrypt', 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'CmdExec', 
		@command=N'powershell.exe -ExecutionPolicy Bypass -file "$(ProgramFiles)\PBIST\SM\PowerShell\Scripts\Encrypt password for SQL agent.ps1" -sqlServer $(SQL_SERVER) -sqlDatabase "$(DatabaseName)" -password "put password here, run, then set this back to empty string" ', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0)
    GOTO QuitWithRollback;
EXEC @ReturnCode = msdb.dbo.sp_update_job @job_id = @jobId, @start_step_id = 1;
